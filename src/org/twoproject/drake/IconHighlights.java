...
//	public static final int TYPE_DOCKBAR=2;
	...
	private static Drawable oldSelector(Context context, int type){
		int selectedColor=AlmostNexusSettingsHelper.getHighlightsColorFocus(context);
		int pressedColor=AlmostNexusSettingsHelper.getHighlightsColor(context);
    	//ADW: Load the specified theme//Fix here !!
    	String themePackage=AlmostNexusSettingsHelper.getThemePackageName(context, Launcher.THEME_DEFAULT);
    	Resources themeResources=context.getResources();

    	
    	//Init about dialog
//    	AlertDialog.Builder about = new AlertDialog.Builder(context);
//        about.setMessage(themePackage);
//        about.setPositiveButton("k", null);
//        about.setTitle("dbg");
//        about.setIcon(R.drawable.ic_launcher_home);
//    	about.show();
		...
		if(themeResources!=null){
			...
			if(type==TYPE_DRAWER){
				int use_drawer_icons_bgs_id=themeResources.getIdentifier("use_drawer_icons_bg", "bool", null/*themePackage*/);
				...
				if(use_drawer_icons_bgs){
					int resource_id=themeResources.getIdentifier("normal_application_background", "drawable", null/*themePackage*/);
					if(resource_id!=0){
...
//					resource_id=themeResources.getIdentifier("dockbar_selector", "drawable", null/*themePackage*/);
//				}else{
					resource_id=themeResources.getIdentifier("shortcut_selector", "drawable", null/*themePackage*/);
//				}
				if(resource_id!=0){
					drawable=themeResources.getDrawable(resource_id);
				}else{
